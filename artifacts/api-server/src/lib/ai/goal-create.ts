...
function parseExtractedGoal(rawText: string): ExtractedGoal | null {
  ...
  const goalType: GoalType = ["habit", "target", "average", "milestone"].includes(String(parsed.goalType)) ? (parsed.goalType as GoalType) : "habit";
...
export async function createGoalFromMessage(
  ...
  if (!extracted) return { response: "I couldn't safely turn that into a goal. Tell me the specific goal you'd like to add.", inputTokens, outputTokens };
  if (!extracted.ready) return { response: "Sure — what specific goal or milestone would you like me to add?", inputTokens, outputTokens };
  ...
  if (cfg.goalCountLimit > 0) {
    ...
    if (goalCount >= cfg.goalCountLimit) {
      return { response: `You've reached the ${cfg.goalCountLimit}-goal limit on your ${cfg.label} plan. You can manage your goals or upgrade from the dashboard.`, inputTokens, outputTokens };
    }
  }
  ...
  await recordActivityEvent({
    ...
    targetValue: extracted.targetValue,
    targetUnit: extracted.targetUnit,
    metadata: { createdFromMessage: true },
  });
  ...
  return { response: `Done — I added “${extracted.title}”${detail}.`, inputTokens, outputTokens };
}
