...
import { ContentEntry, ContentKind, contentEntries, contentPath, getContent, getContentByKind } from "@/content/all-content";
...
function Breadcrumbs({ entry }: { entry?: ContentEntry }) {
  return <nav aria-label="Breadcrumb" className="text-sm text-muted-foreground mb-8">
    ...
    {entry ? <><Link href={roots[entry.kind]} className="hover:text-foreground">{labels[entry.kind]}</Link><span className="mx-2">/</span><span className="text-foreground">{entry.title}</span></> : <span className="text-foreground">Resources</span>}
  </nav>;
}
...
function RelatedContent({ entry }: { entry: ContentEntry }) {
  ...
  return <section className="mt-14 border-t pt-10"><h2 className="text-2xl font-semibold mb-5">Read next</h2><div className="grid gap-4 md:grid-cols-2">{related.map((item) => <Link key={`${item.kind}-${item.slug}`} href={contentPath(item)} className="block rounded-xl border p-5 hover:bg-muted/40"><div className="text-xs uppercase tracking-wide text-muted-foreground mb-2">{item.category}</div><div className="font-semibold">{item.title}</div><p className="text-sm text-muted-foreground mt-2">{item.description}</p></Link>)}</div></section>;
}
