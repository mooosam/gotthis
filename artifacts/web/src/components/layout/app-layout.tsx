...
function Sidebar({ className, isAdmin, whatsappConnected }: { className?: string; isAdmin: boolean; whatsappConnected: boolean }) {
  ...
  return <div className={className} style={{userSelect:"none"}}><div className="px-5 pt-7 pb-3"><ProfileButton whatsappConnected={whatsappConnected}/></div><nav className="flex-1 px-3 mt-3 overflow-y-auto">
    ...
    {categories.length>0&&<div style={{marginTop:24,marginBottom:8}}><div style={{fontSize:10,fontWeight:700,letterSpacing:"0.1em",color:"#9CA3AF",padding:"0 10px",marginBottom:8,textTransform:"uppercase"}}>Goals</div>{categories.map(cat=><Link key={cat} href="/goals"><div className="flex items-center gap-2.5 px-2.5 py-[5px] rounded-lg mb-[2px] cursor-pointer transition-colors hover:bg-gray-50 dark:hover:bg-sidebar-accent" style={{fontSize:13,color:"#6B7280"}}><span style={{width:7,height:7,borderRadius:"50%",background:getCategoryColor(cat),flexShrink:0,display:"inline-block"}}/><span style={{textTransform:"capitalize"}}>{cat}</div></Link>)}</div>}
  </nav><div className="px-3 pb-4 mt-2"><SidebarLogoutButton/></div></div>;
}

function ProfileButton({whatsappConnected}:{whatsappConnected:boolean}) { const {user}=useUser(); const {signOut}=useClerk(); const [,setLocation]=useLocation(); const {theme,setTheme}=useTheme(); if(!user)return <div className="space-y-2"><Skeleton className="h-8 w-8 rounded-full"/><Skeleton className="h-3 w-24"/></div>; const initials=user.fullName?user.fullName.split(" ").map(n=>n[0]).join("").substring(0,2).toUpperCase():(user.primaryEmailAddress?.emailAddress?.substring(0,2).toUpperCase()??"U"); return <DropdownMenu><DropdownMenuTrigger asChild><button data-testid="user-menu-trigger" className="w-full text-left focus:outline-none group"><div className="flex items-center gap-2.5"><Avatar className="h-8 w-8 rounded-full flex-shrink-0"><AvatarImage src={user.imageUrl}/><AvatarFallback className="rounded-full bg-[#1C1C1E] text-white text-xs font-semibold">{initials}</AvatarFallback></Avatar><span className="font-semibold truncate" style={{fontSize:14,color:"#111827",maxWidth:120}}>{user.fullName||user.primaryEmailAddress?.emailAddress?.split("@")[0]||"User"}</span></div><div className="flex items-center gap-1.5 mt-1.5 pl-[42px]"><span style={{width:7,height:7,borderRadius:"50%",background:whatsappConnected?"#22C55E":"#D1D5DB",display:"inline-block",flexShrink:0}}/><span style={{fontSize:12,fontWeight:500,color:whatsappConnected?"#16A34A":"#9CA3AF"}}>{whatsappConnected?"Connected":"Not connected"}</span></div></button></DropdownMenuTrigger><DropdownMenuContent align="start" side="bottom" className="w-52" data-testid="user-menu-content"><DropdownMenuLabel className="text-xs text-muted-foreground font-normal">{user.primaryEmailAddress?.emailAddress}</DropdownMenuLabel><DropdownMenuSeparator/><DropdownMenuItem onClick={()=>setLocation("/account")} className="cursor-pointer"><Settings className="mr-2 h-4 w-4"/>Settings</DropdownMenuItem><DropdownMenuItem onClick={()=>setTheme(theme==="dark"?"light":"dark")} className="cursor-pointer">{theme==="dark"?<Sun className="mr-2 h-4 w-4"/>:<Moon className="mr-2 h-4 w-4"/>}{theme==="dark"?"Light mode":"Dark mode"}</DropdownMenuItem><DropdownMenuSeparator/><DropdownMenuItem onClick={()=>signOut()} className="cursor-pointer text-destructive focus:text-destructive"><LogOut className="mr-2 h-4 w-4"/>Log out</DropdownMenuItem></DropdownMenuContent></DropdownMenu>; }

function MobileNav({isAdmin}:{isAdmin:boolean}) { return <header className="flex md:hidden h-14 items-center justify-between px-4 border-b border-[#EBEBEB] dark:border-border bg-white dark:bg-sidebar"><span className="text-base font-semibold tracking-tight" style={{color:"#111827"}}>GotThis</span><Sheet><SheetTrigger asChild><Button variant="ghost" size="icon" className="md:hidden"><Menu className="h-5 w-5"/><span className="sr-only">Toggle menu</span></Button></SheetTrigger><SheetContent side="left" className="w-52 p-0 bg-white dark:bg-sidebar border-r border-[#EBEBEB] dark:border-border"><Sidebar className="flex h-full flex-col" isAdmin={isAdmin} whatsappConnected={false}/></SheetContent></Sheet></header>; }
